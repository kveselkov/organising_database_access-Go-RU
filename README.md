...
Мы сможем использовать пример внедрения зависимостей немного позже. Давайте изменим пакет моделей так, что бы он возвращал пользовательский тип БД (который включает *sql.DB) и внедрим логику базы данных в виде типа DB.
...
Мы можем использовать данный интерфейс во всем нашем приложении.  

We can then use this interface instead of the direct DB type throughout our application. Обновленный пример:
...
Лично я не фанат переменных уровня приложения в контексте в области видимости запроса - он выглядит неуклюжим и обременительным для меня. Документация x/net/context так же советует:

> Используйте значения контекста только для области видимости данных внутри запроса, которые переходят между процессами и точеками входа API, а не для передачи необязательных параметров функции.  <blockquote></blockquote>

Тем не менее, люди используют данный подход. И если ваш проект содержит множество пакетов - и использование глобальной конфигурации не обсуждается - то это давольно привлекательно предложение.
